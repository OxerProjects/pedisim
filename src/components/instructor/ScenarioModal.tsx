...
import { Vitals } from '@/stores/simulatorStore';
        ...
        >
            <div className="bg-[#1a1a1a] border border-[#313131] rounded-xl w-[800px] max-h-[90vh] flex flex-col overflow-hidden shadow-2xl">
                ...
                <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
                    <div className="flex flex-col gap-2">
                        <label className="text-sm text-gray-400 font-bold">
                            {lang === 'HE' ? 'שם התרחיש:' : 'Scenario Name:'}
                        </label>
                        <input
                            type="text"
                            value={name}
                    ...
                    <div className="flex flex-col gap-2">
                        <label className="text-sm text-gray-400 font-bold">
                            {lang === 'HE' ? 'תיאור כללי (למשתמש):' : 'General Description:'}
                        </label>
                        <textarea
                            value={description}
                    ...
                    <div className="border-t border-gray-800 pt-6">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-lg font-bold text-white">
                                {lang === 'HE' ? 'שלבים (מקסימום 4)' : 'Phases (Max 4)'}
                            </h3>

                            {phases.length < 4 && (
                                <button
                                    onClick={addPhase}
                                    className="bg-gray-800 hover:bg-gray-700 px-3 py-1 text-sm rounded flex items-center gap-1 font-bold"
                                >
                        ...
                        <div className="flex flex-col gap-4">
                            {phases.map((p, idx) => (
                                <div
                                    key={p.id}
                                    className="border border-gray-700 rounded bg-[#111] p-4 flex flex-col gap-4"
                                >
                                    ...
                                    <div className="grid grid-cols-4 gap-2">
                                        {(['heartRate', 'spO2', 'nibpSys', 'nibpDia'] as (keyof Vitals)[]).map((k) => (
                                            <div key={k} className="flex flex-col gap-1">
                                                <span className="text-xs text-gray-400">{k}</span>
                                                <input
                                                    type="number"
                                                    value={p.targetVitals[k]}
                                                    onChange={(e) => updatePhaseVital(idx, k, Number(e.target.value))}
                ...
                <div className="p-4 border-t border-[#313131] bg-[#0e0e0e] flex justify-end gap-2">
                    <button
                        onClick={onClose}
                        className="px-4 py-2 rounded text-gray-300 hover:bg-gray-800 transition"
                    >
                        {lang === 'HE' ? 'ביטול' : 'Cancel'}
                    </button>
                    <button
                        onClick={handleSave}
                        className="px-6 py-2 rounded bg-blue-600 hover:bg-blue-500 text-white font-bold transition"
                    >
